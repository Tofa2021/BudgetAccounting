    ...
    public Response handle(RequestAction action, AuthorizedRequest request, Long userId) {
        return switch (action) {
            case CREATE_HOUSEHOLD_MEMBER ->
                    Response.created(dtoMapper.toDTO(householdMemberService.create((CreateMemberHouseholdRequest) request), HouseholdMemberDTO.class));
            ...
            case DELETE_HOUSEHOLD_MEMBER -> {
                householdMemberService.delete((DeleteMemberHouseholdRequest) request);
                yield Response.noContent();
            }
