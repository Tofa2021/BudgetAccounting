...
import org.example.model.DTOConvertible;
...
public class Controller {
    ...
    private Response processAuthorizedRequest(RequestAction action, AuthorizedRequest request) {
        ...
        Long userId = extractUserIdFromToken(request.getToken());
    ...
    private boolean isValidToken(String token) {
        return jwtProvider.isValidateAccessToken(token);
    }

    private Long extractUserIdFromToken(String token) {
        return Long.parseLong(jwtProvider.getAccessClaims(token).getSubject());
    }
